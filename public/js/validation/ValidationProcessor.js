...
class ValidationProcessor extends ValidationProcessorBase {
  ...
  checkNoErrors(inputErrorsArray) {
    if (!Array.isArray(inputErrorsArray)) {
      throw new TypeError(
        'You need to pass an array that contains InputErrors instances'
      )
    } else {
      for (let inputErrorsItem of inputErrorsArray) {
        if (!inputErrorsItem?.[IS_INPUT_ERRORS_INSTANCE]) {
          throw new TypeError(
            'You need to pass an array that contains InputErrors instances'
          )
        }
    ...
    const nullOnlyInputErrors = inputErrorsArray.filter((inputErrors) => {
      return inputErrors.checkAllNull()
    })

    if (nullOnlyInputErrors.length === inputErrorsArray.length) {
      return true
    } else {
      return false
    }
